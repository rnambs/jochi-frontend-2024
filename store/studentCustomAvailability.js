...
const actions = {
    async teacherAvailableSlot({ commit }, payLoad) {
        ...
        try {
            const response = await this.$axios.$get(BASE_URL + `custom/availability/student_available_slot?student_id=${payLoad.teacher_id}`, {
                headers: {
                    'Authorization': ` ${token}`
                },
            });
    ...
    async updateTeacherAvailability({ commit }, payLoad) {
        ...
        const token = localStorage.getItem('token')
        try {
            const response = await this.$axios.$post(BASE_URL + 'custom/availability/students', payLoad, {
                headers: {
                    'Authorization': ` ${token}`
                },
            });
