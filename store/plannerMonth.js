...
const actions = {
  async getMonthlyPlanner({ commit }, payLoad) {
    try {
  ...
  async updateAssignment({ commit }, payLoad) {
    ...
    try {
      ...
      else if (e.response.data.message == "Please select an upcoming date") {
        commit('setSuccessMessage', "");
        commit('setSuccessType', "");
        commit('setErrorMessage', "Please select an upcoming date");
        commit('setErrorType', "error");
      }
      else if (e.response.data.message == "Validation error") {
        commit('setSuccessMessage', "");
